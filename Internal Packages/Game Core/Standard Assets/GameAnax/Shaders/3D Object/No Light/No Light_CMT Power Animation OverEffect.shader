/*
GameAnax/No Light/CMT Power Animation OverEffect
================================================
...
Shader "GameAnax/No Light/CMT Power Animation OverEffect" {
	Properties {
		_Color ("Color", Color) = (1,1,1,1)
		_MainTex ("Base (RGB) A (Alpha)", 2D) = "white" {}
		_ActiveTex ("ActiveTex (RGB) A (Alpha)", 2D) = "white" {}
		_AnimTex ("A (Alpha)", 2D) = "white" {}
		_CutOff("Animation Pos",float) = 0
	}
	SubShader {
		...
		Pass{
			...
			fixed4 frag (v2f_img i) : COLOR {
				...
				fixed f = texAnim.a > _CutOff;

				texFinal=texMain;

				texFinal = lerp(texMain, texActive, texActive.a);
				texFinal = lerp(texFinal, texMain, f);
