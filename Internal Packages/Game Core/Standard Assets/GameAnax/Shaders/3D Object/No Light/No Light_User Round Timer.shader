...
Shader "GameAnax/No Light/User Round Timer with Ring & Color" {
	Properties {
		_Color ("Color", Color) = (1,1,1,1)

		_MainTex ("Base (RGB) A (Alpha)", 2D) = "white" {}
		_UserMaskTex ("Base (RGB) A (Alpha)", 2D) = "white" {}
		_UserRingTex ("Base (RGB) A (Alpha)", 2D) = "white" {}
	...
	SubShader {
		...
		Pass{
			...
			uniform sampler2D _UserMaskTex;
			uniform sampler2D _UserRingTex;
			...
			fixed4 frag (v2f_img i) : COLOR {
				...
				fixed4 texMaskOff = tex2D(_UserMaskTex,i.uv);
				fixed4 texRing = tex2D(_UserRingTex,i.uv);
				...
				if(texAnim.a < _CutOff) {
					texFinal = texRing * _NVColor;
				}else{
					texFinal = texRing * _VColor;
				}

				texFinal = lerp(texMain,texFinal,texFinal.a);
